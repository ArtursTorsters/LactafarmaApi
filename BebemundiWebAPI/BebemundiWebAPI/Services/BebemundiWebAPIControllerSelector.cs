...
{
    public class BebemundiWebAPIControllerSelector: DefaultHttpControllerSelector
    {
        private HttpConfiguration _config;
        public BebemundiWebAPIControllerSelector(HttpConfiguration config):base(config)
        {
            _config = config;
        }
        ...
        private string GetVersionFromMediaType(HttpRequestMessage request)
        {
            ...
            var ex = new Regex(@"application\/vnd\.bebemundiwebapi\.([a-z]+)\.v([0-9]+)\+json", RegexOptions.IgnoreCase);
        ...
        {
            const string HEADER_NAME = "X-BebemundiWebAPI-Version";
